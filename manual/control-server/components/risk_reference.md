...
프로덕션 `GridFuser`는 짧은 채널 인계 공백에 객체가 깜박이지 않도록 미관측 트랙을 최대 5개
윈도우 동안 마지막 좌표로 월드 프레임에 유지한다. 이 coast 객체도 이후 zone·Risk 단계를 통과하므로,
경보는 짧게 보수적으로 유지될 수 있다. `ConcatFuser`는 미관측 상태를 재연결에만 사용한다.
