...
| 상황 | 동작 |
|------|------|
| 같은 채널이 윈도우 내 여러 번 도착 | CCTV `ts`와 무관하게 마지막으로 도착한 스냅샷을 유지 |
| 어떤 채널도 도착하지 않음 | 윈도우가 마감되지 않음 (아래 2.1) |
| 일부 채널만 도착 | 이번 윈도우에 도착한 채널을 모두 전달 |
| 콜백 미등록 상태로 마감 | 해당 윈도우 데이터 폐기 + 에러 로그 |

집계기는 CCTV `ts`로 프레임 순서를 판정하지 않는다. 단일 FIFO 수신 경로의 도착 순서를 사용하므로
동일 timestamp나 장치 시계 보정이 현재 윈도우의 채널 데이터를 비우지 않는다. `windowSizeMs`는
control-server의 집계 주기이며 CCTV 간 timestamp 허용 오차가 아니다.
