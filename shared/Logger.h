 ...
 * 각 호출부는 자신의 인터페이스 이름을 kIface로 들고 있다가
 * logSuccess/logError/logDebug 첫 인자로 넘기면 됨
 *
 * @note [ 왜 shared/ 인가 ]
 * 예전에는 compute-server/src/log/Logger.h와 control-server/src/log/Logger.h에
 ...
 *
 * @note [ 왜 콘솔까지 비동기인가 ]
 * 예전에는 CSV 기록만 비동기였고 콘솔은 호출 스레드에서 직접 std::cout/std::cerr로 썼음
 * -- 특히 std::cerr는 unit-buffered라 logError 한 번마다 write() syscall이 발생했고,
 *    이게 프레임마다 도는 경로(발행 성공, 팬텀 객체 제거 등)에 그대로 걸려 있었음
 * -- 이제 콘솔/파일 모두 같은 메모리 큐에 넣기만 하고 백그라운드 워커가 배치로 내보냄
 *    → 파이프라인 스레드에서 로깅 때문에 발생하는 I/O가 0
 *
 * @warning [ 크래시 시 유실 ]
...
/// @brief 로거 동작 설정 (AppConfig에서 채워 main이 initLogger로 주입)
struct LogConfig {
     ...
     * @details 파일 회전은 logrotate가 담당하므로 이름은 반드시 고정이어야 한다.
     *          예전처럼 날짜별 파일명(YYYY-MM-DD.csv)을 쓰면 logrotate의 rename/postrotate와
     *          이중으로 회전해 서로 어긋난다
     */
    std::string fileName = "veda.csv";
};

/// @brief 로깅 구현 세부사항 (외부에서는 logDebug/logSuccess/logError/initLogger만 쓰면 됨)
...
struct LogEntry {
     ...
     *          ("YYYY-MM-DD-HH:MM:SS"는 19자라 libstdc++ SSO(15자)를 넘겨 매번 malloc이 났음)
     */
 ...
 *
 * @details
 * - enqueue()는 메모리 큐에 push만 하고 즉시 반환 (큐 포화 경고를 제외하면 I/O 없음)
 * - 백그라운드 스레드가 주기적으로 큐 전체를 스왑해서 콘솔 + 파일에 한 번에 씀
 * - 프로그램 종료 시(정적 소멸자) 남은 로그까지 마지막으로 flush 후 종료
 * - 타임스탬프 포맷팅은 워커가 초 단위 캐시로 수행 (배치당 localtime_r/strftime 1~2회)
 *
 * @note [ 파일 회전은 이 클래스가 하지 않음 ]
 * 고정 이름(LogConfig::fileName) 하나에만 append하고, 회전/압축/보존은 logrotate에 맡긴다.
 * logrotate가 create 방식으로 파일을 rename한 뒤 SIGHUP을 보내면 requestReopen()이 호출되고,
...
public:
    ...
    /**
     * @brief   CSV를 닫았다 다시 열도록 요청 (logrotate의 postrotate SIGHUP에서 호출)
     *
     * @details
     * ofstream은 워커 스레드 전용이라 여기서 직접 만지지 않고 플래그만 세운다 → 파일 I/O 에
     * 락을 추가하지 않고도 안전하며, 시그널 감시 스레드는 즉시 반환한다.
     * 실제 close/open은 워커가 다음 주기에 수행 (진행 중인 배치를 먼저 내보낸 뒤)
     */
...
private:
    ...
    /**
     * @brief   time_point를 "YYYY-MM-DD-HH:MM:SS" 로 포맷 (초 단위 캐시)
     *
     * @details
     * 한 배치는 flushInterval(기본 500ms) 이내라 대부분의 항목이 같은 초를 공유한다.
     * 같은 초면 직전 결과를 그대로 재사용해 localtime_r(glibc 전역 락)/strftime 호출을
     * 배치당 1 ~ 2회로 줄인다. assign()은 기존 capacity를 재사용하므로 힙 할당도 없다.
     *
    ...
    /// @brief 현재 CSV를 flush 후 닫음. 다음 writeFile이 (logrotate가 새로 만든) 파일을 연다.
    void closeFile() {
        if (ofs_.is_open()) {
            ofs_.flush();
            ofs_.close();
    ...
    /// @brief 고정 이름 파일이 닫혀 있으면 열고, batch를 한 번에 씀 (회전은 logrotate 담당)
    void writeFile(const std::deque<LogEntry>& batch) {
        ...
        for (const auto& e : batch) {
            ...
            // (csvEscape는 값을 복사해 반환하므로, 여기서 부르면 항목마다 불필요한 할당이 생김)
        ...
        while (!stopping_) {
            std::deque<LogEntry> batch;
            {
                ...
                cv_.wait_for(lk, std::chrono::milliseconds(flushIntervalMs_.load(std::memory_order_relaxed)),
                             [this] { return stopping_.load() || reopenRequested_.load(std::memory_order_acquire); });
                batch.swap(pending_);
            }
            writeBatch(batch);

            // 배치를 먼저 내보낸 뒤에 닫는다 → 회전 직전에 쌓인 로그가 유실되지 않음
            // (logrotate가 rename한 옛 파일에 기록되는 게 맞는 항목들이므로)
...
inline void emit(const char* iface, LogLevel level, const std::string& msg) {
    ...
    LogSink::instance().enqueue({std::chrono::system_clock::now(), iface != nullptr ? iface : "", level, msg});
}

}  // namespace log_detail

/**
 * @brief   로거 설정을 적용 (main이 config를 읽은 직후 1회 호출)
...
/**
 * @brief   로그 파일을 닫았다 다시 열도록 요청 (logrotate의 postrotate SIGHUP 핸들러에서 호출)
 * @details 실제 close/open은 워커 스레드가 다음 주기에 수행하므로 이 호출은 즉시 반환된다.
