...
ThresholdRiskPolicy::ThresholdRiskPolicy(std::shared_ptr<IDistanceMetric> metric, const RiskConfig& risk,
    ...
    // 조립 시점 fail-fast. AppConfig 가 일부를 보정하지만 DI 로 직접 넣는 경로는 막지 못한다.
    if (!metric_) {
    ...
    // Danger/Warning 어느 쪽도 성립하지 않는다. 음수도 마찬가지로 영원히 성립하지 않는다.
    ...
    // 역전되면 Warning 이 영영 도달 불가능해진다 (dangerous 이하가 전부 Danger 로 먹힘).
    // 과다 경보라 안전 방향이긴 하나, 운영자가 의도한 2단계 판정이 조용히 1단계가 된다.
...
void ThresholdRiskPolicy::evaluate(domain::WorldFrame& frame, domain::RiskEvaluation& out) {
    ...
    // resize 는 정상 운용에서 매 프레임 같은 값이라 사실상 no-op 이다 (호출자가 버퍼를 재사용).
    ...
    // 기존에도 있던 초기화 루프에 검사와 SoA 구축을 얹었다. 순회 횟수는 그대로 N 이다.
    //
    // [ 유한성 검사를 여기서 하는 이유 ]
    // NaN 좌표는 거리도 NaN 으로 만들고, NaN 은 '<' 와 '<=' 모두에서 false 다. 그러면
    ...
    if (droppedThisFrame > 0) {
        ...
        if (nonFiniteCount_ == 1 || nonFiniteCount_ % 100 == 0) {
            logError(kIface, "비유한 좌표 객체 " + std::to_string(droppedThisFrame) +
                                 "개를 판정에서 제외 (누적 " + std::to_string(nonFiniteCount_) + "건)");
        }
    }
    ...
    for (std::size_t k = 0; k < candIdx_.size(); ++k) {
        ...
        // 좌표만 담긴 연속 배열을 훑는다. metric_ 호출은 유지 -- 거리 정의를 DI 로 바꿀 수
        // 있다는 것이 이 계층의 계약이므로, 여기서 유클리드를 인라인하면 그 seam 이 죽는다.
        ...
        // 문자열 조립은 rate-limit 을 통과한 뒤에만 한다 (핫패스 할당 방지).
        ++riskLogCount_;
        if ((riskLogCount_ == 1 || riskLogCount_ % 50 == 0) && isLogEnabled(LogLevel::Info)) {
            logSuccess(kIface, "gid=" + std::to_string(vehicle.gid) + " " +
                                   ...
                                   std::to_string(nearest.gid) + ", 거리=" + std::to_string(minDist) + "m, 누적 " +
                                   std::to_string(riskLogCount_) + "건)");
        }
        ...
        //
        // 인덱스를 그대로 쓴다 -- 예전에는 gid 로 다시 전체 순회해서 찾았고, 그 재탐색이
        // 위험 차량마다 O(N) 이었다.
