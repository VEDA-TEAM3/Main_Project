 ...
 * 각 호출부는 자신의 인터페이스 이름(예: "Aggregator", "Fuser")을 kIface 등으로 들고 있다가
 * logSuccess/logError 첫 인자로 넘기면 됨
 *
 * 콘솔 출력과 동시에 같은 내용을 "YYYY-MM-DD.csv" 파일에도 남김 (날짜가 바뀌면 새 파일로 전환)
 * 파일 I/O가 호출 스레드를 막지 않도록, 로그는 우선 메모리 큐에만 쌓고
 * 백그라운드 스레드가 주기적으로 큐 전체를 모아 한 번에 파일에 씀 (배치 기록)
 *
 * @note    compute-server/src/log/Logger.h 와 동일한 설계 (두 서버가 로그 포맷을 공유)
 */
