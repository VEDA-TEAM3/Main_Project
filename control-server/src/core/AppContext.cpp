...
std::shared_ptr<Controller> AppContext::buildController() {
    ...
    auto fuser = std::make_shared<ConcatFuser>();
    auto zoneMapper = std::make_shared<NullZoneMapper>();
    auto riskPolicy =
        std::make_shared<ThresholdRiskPolicy>(metric, config_.risk.warningDistance, config_.risk.dangerousDistance);
    auto dispatcher = std::make_shared<ConsoleDispatcher>();
    auto sink = std::make_shared<ConsoleSink>();

    return std::make_shared<Controller>(receiver, aggregator, fuser, zoneMapper, riskPolicy, dispatcher, sink);
}