 ...
 * @details
 * MQTT 브로커를 구독하여 다수의 채널에서 비동기적으로
 * 날아오는 프레임 데이터를 받아 다음 파이프라인(Aggregator)으로 넘김.
 */
...
public:
    ...
    /**
     * @brief 수신 시작
     * @note 연결 실패 시 예외를 던지지 않음. 구현체 내부에서 재시도 로직을 수행하며,
     *       재시도 정책(간격/횟수)은 Config. 연결 상태는 alive 콜백으로 상위에 전파
     */
