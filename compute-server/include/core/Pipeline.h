 ...
 * imageMapScale/Offset을 Blur 정합용으로 조정하는 순간 월드 좌표가 조용히 틀어짐
 * -- 기본값이 항등(scale=1, offset=0)이라 증상이 드러나지 않는 잠복 결함이었음
 *
 * @note [ Risk를 Blur보다 먼저 처리하는 이유 ]
 ...
 * @details 문자열 ↔ 열거형 변환은 AppContext가 담당
 */
...
private:
     ...
     *
     * @details
     * 라우터가 값 반환 대신 이 버퍼를 out-parameter로 채운다. clear()는 capacity를
     * 유지하므로 warmup 이후에는 Blur/Risk 벡터의 힙 할당이 0이 된다.
     * -- 예전에는 프레임마다 RouteResult를 새로 만들어 reserve 했고, 그것이 per-frame
    ...
    /// @name 폐기 사유별 누적 카운터 (로그 rate-limit 용, onPacket은 단일 스레드 호출 전제)
    /// @{
