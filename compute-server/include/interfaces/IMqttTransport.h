 ...
 *
 * @details
 * Sink는 무엇을 언제 보낼지(큐잉 정책, 유효성 검사, 토픽/QoS)를 정하고,
 * 이 인터페이스의 구현체는 어떻게 보낼지(TCP/TLS 연결, 재접속, mosquitto 루프)만 담당함
 *
 * @note [ 하나의 연결을 여러 Sink가 공유하는 이유 ]
 * 예전에는 MqttBlurSink와 MqttTopViewSink가 각자 mosquitto 클라이언트를 만들어
 * 채널당 TLS 커넥션 2개 + mosquitto 네트워크 스레드 2개를 썼음 (4채널이면 커넥션 8개)
 * OpenSSL 세션 버퍼가 커넥션당 수십 KB라 라즈베리파이에서는 프레임 버퍼보다 비중이 컸고,
 * 두 Sink가 각자 mosquitto_lib_init()/mosquitto_lib_cleanup()을 부르는 바람에
 * 먼저 파괴된 쪽이 전역 상태를 해제해버리는 종료 시 UB도 있었음
 * → 연결을 하나로 모으고, 라이브러리 초기화는 참조 카운트로 관리
 *
 * @note [ 큐는 왜 공유하지 않는가 ]
 * 전송 큐와 워커 스레드는 Sink마다 따로 둠
 * Blur 쪽이 밀린다고 Risk 발행이 함께 지연되면 안 되기 때문
 */
...
public:
     ...
     * @return  removeConnectionListener에 넘길 핸들
     *
     * @warning [ 스레드 ]
     ...
     *
     * @details
     * 리스너는 보통 Sink의 this를 캡처하는데, Sink는 transport보다 먼저 파괴됨
     ...
     * @details 최초 접속에 실패해도 백그라운드에서 재시도하므로 false여도 치명적이지 않음
     ...
     * @return  mosquitto가 큐잉을 받아들였으면 true
     *
     * @warning 예외를 던지지 않음 (실패는 false로만 알림)
     */
    virtual bool publish(std::string_view topic, std::string_view payload, int qos, bool retain = false) noexcept = 0;

    /**
     * @brief   클라이언트가 만들어지고 루프가 도는 중인가
     * @warning isConnected()와 동일한 락 프리 요구가 적용됨
     */
     ...
     * 그 콜백은 다시 queueMutex_를 잡는다. 따라서 이 함수가 구현체 내부 락을 잡는 순간
     * 다음 순환이 성립한다:
     *
     ...
     *   => 교착
     *
     * MqttTransport는 std::atomic_bool의 단순 load로 구현하여 이 제약을 지킨다.
     * 새 구현체도 원자적 읽기만 사용할 것
