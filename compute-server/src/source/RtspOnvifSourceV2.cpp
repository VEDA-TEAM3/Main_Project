...
void RtspOnvifSourceV2::stop() noexcept {
    ...
    // (= 프로세스가 SIGINT/SIGTERM에 응답하지 못하고 MQTT 종료 신호도 못 보냄)
    {
...
void RtspOnvifSourceV2::workerLoop() {
    ...
    while (!stopping_) {
        // client보다 먼저 선언 → client가 먼저 파괴되므로, 콜백이 참조하는 동안 항상 살아있음
        ...
        client.onPayloadReceived = [this, &sessionProductive](std::string_view payload) {
            // 실제로 payload를 받은 세션만 생산적으로 본다. (백오프 초기화의 유일한 기준)
        ...
        // stop()이 이 세션을 취소할 수 있도록 등록 (파괴 전에 반드시 해제해야 UAF가 없음)
        {
            std::lock_guard<std::mutex> lk(clientMutex_);
            if (stopping_) {
                break;
        ...
        // PLAY가 200이 아니면 스트리밍이 시작되지 않으므로 run()에 들어가지 않는다
        ...
        // 다음 실패에 대비해 증가 (backoffMaxSec_에서 포화)
...
bool RtspOnvifSourceV2::next(domain::RawPacket& out) {
    std::string report;
    {
        ...
        // 복사 대신 버퍼 소유권 교환(O(1) 포인터 스왑): out이 들고 있던(이미 소비된) 버퍼가
        // 슬롯으로 넘어가 다음 write의 capacity로 재사용됨 → per-frame memcpy 제거 + capacity 보존
        // 슬롯/파이프라인 모두 mtx_ 안에서만 bytes를 만지므로 스왑은 스레드 안전함
