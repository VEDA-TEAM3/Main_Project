 ...
 * @brief   INetwork 를 IMetadataSource 로 변환하는 어댑터
 */
...
/**
 * @brief   RtspClient의 push 콜백을 IMetadataSource::next() 의
 *          pull 인터페이스로 변환하는 어댑터
 *
 * @details
 * RtspClient 의 connect -> setup -> play -> run 생명주기를 별도 스레드에서 돌림.
 * onPayloadReceived 콜백으로 들어오는 payload 를 스레드 세이프 큐에 쌓고,
 * next() 는 그 큐에서 꺼냄.
 *
 * 연결이 끊기면(run() 이 리턴하면) 지수 백오프로 재연결
 * (1s -> 2s -> 4s -> ... 최대 30s, 성공 시 리셋).
 * next() 는 재연결 시도 중에도 블로킹 대기할 뿐 false 를 반환하지 않음
 *
 * @todo
