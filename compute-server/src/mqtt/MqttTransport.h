 ...
 *
 * @note [ 채널 생존 신호 (LWT) ]
 * veda::topic::alive(ch)에 retained로 "1"(살아있음)/"0"(죽음)을 실어보냄
 ...
 *
 * 이 신호가 있어야 control-server가 객체가 없는 빈 프레임과 채널이 죽어서
 * 아무것도 안 오는 상태를 구분할 수 있음
 *
 * 연결 자체를 소유한 이 클래스가 담당함
 * -- 유언은 접속 이전에 걸어야 하고 되살아남
 * 통지는 (재)접속 콜백에서 해야 하므로 Sink 쪽에서는 할 수 없는 일
 */
...
private:
    ...
    /// @brief client_ 수명 보호 (publish는 sink 워커 스레드들에서, destroy는 종료 시점에 일어남)
    ...
    /**
     * @brief   리스너 목록 보호
     * @details 콜백 실행 중에도 계속 잡고 있음 → removeConnectionListener()가 반환하면
