...
/// @details lib_init/lib_cleanup은 전역 상태를 다루므로 마지막 사용자가 사라질 때만 정리
/// @{
...
/// @name 채널 생존 신호 페이로드 ("1"=alive, "0"=dead)
/// @{
...
void MqttTransport::removeConnectionListener(ListenerId id) noexcept {
    ...
    // notifyListeners()가 콜백 실행 내내 listenerMutex_를 잡고 있으므로,
...
bool MqttTransport::initializeClient() noexcept {
    ...
    // [초기화 경합 차단] 설정이 끝나기 전에는 멤버(client_)에 공개하지 않는다.
    // 로컬 포인터로 완전히 구성한 뒤에야 clientMutex_ 아래에서 대입
    // → publish()가 반쯤 설정된 클라이언트를 보거나,
    // 설정 호출과 mosquitto_publish가 동시에 같은 핸들을 만지는 일이 없음
    mosquitto* client = mosquitto_new(clientId_.c_str(), true, this);
    if (client == nullptr) {
        logError(kIface, "mosquitto_new 실패");
        destroyClient();  // client_는 아직 nullptr → 라이브러리 참조만 반납
        return false;
    }
    ...
    // LWT: 접속 이전에 걸어야 브로커가 유언으로 등록함 (접속 후에는 늦음)
    ...
    // loop_start 이전에 공개해야 onConnect 콜백의 생존신호 publish()가 client_를 찾을 수 있음
    {
...
void MqttTransport::stop() noexcept {
    ...
    // retryMutex_를 한 번 잡았다 놓은 뒤 notify: 그냥 notify만 하면 retryLoop가 술어를 false로
    // 평가한 뒤 wait_for에 진입하기 전 구간에 알림이 끼어들어 놓치고, wait_for 타임아웃
    // (mqttRetryIntervalMs)만큼 stop()이 지연된다.
    ...
    // -- LWT는 브로커가 keepalive 만료를 감지해야 발행되므로 최대 1.5 * keepalive만큼 늦음
...
void MqttTransport::notifyListeners(bool connected) noexcept {
    ...
    // -- 복사 후 호출하면 그 사이에 Sink가 파괴되어 죽은 객체를 부를 수 있음
...
void MqttTransport::onConnect(mosquitto*, void* userData, int resultCode) {
    ...
    if (connected) {
        ...
        // control-server가 이 채널을 되살아난 것으로 인식함
