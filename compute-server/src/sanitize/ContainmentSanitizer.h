 ...
 *
 * @details
 * - 규칙 A: RISK 후보가 부위 객체(Head/LicensePlate)와 IoU가 iouThresh_를 넘으면 제거
 *          CCTV가 같은 신체 부위를 Head로도 Human으로도 동시에 내보내는 경우를 잡음
 ...
 *
 *   -- 클래스가 다르면(Human이 Vehicle 안에 겹치는 경우 등) 서로 다른 실제
 *   객체일 수 있으므로 적용하지 않음
 *
 * - 규칙 C: likelihood로는 필터링하지 않음
