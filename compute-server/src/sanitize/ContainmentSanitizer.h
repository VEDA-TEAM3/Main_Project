 ...
 *
 * @details
 * - 규칙 A: RISK 후보가 부위 객체(Head/LicensePlate)와 IoU 가 iouThresh_ 를 넘으면 제거
 *          CCTV가 같은 신체 부위를 Head 로도 Human 으로도 동시에 내보내는 경우를 잡음
 *          (실측 덤프에서 확인: ObjectId 3024 ↔ Head 3022, IoU ≈ 0.69)
 * - 규칙 B: 더 작은 RISK 후보가 같은 클래스의 더 큰 RISK 객체 안에
 *          거의 완전히 포함되면(IoMin > containThresh_) 작은 쪽을 제거
 *          부위 객체가 함께 나오지 않는 중복에 대한 안전망
 *
 *   -- 클래스가 다르면(Human 이 Vehicle 안에 겹치는 경우 등) 서로 다른 실제
 *   객체일 수 있으므로 적용하지 않음
 *
 * - 규칙 C: likelihood 로는 필터링하지 않음
