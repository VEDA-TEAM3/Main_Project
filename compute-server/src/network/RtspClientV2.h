...
public:
     ...
     * 깨운다. → run()이 recv 타임아웃을 기다리지 않고 즉시 빠져나오므로 워커 join이 지연되지 않는다.
     * 취소 플래그만 있으면 스트림이 살아있는 동안 recv()가 계속
     * 성공해 run()이 영영 반환하지 않으므로, shutdown()이 반드시 함께 필요하다.
     *
     * @warning close()가 아니라 shutdown()이어야 한다. fd의 소유와 close는 소유자 스레드
     *          (소멸자)의 몫이며, 다른 스레드가 close하면 fd 번호 재사용 경합이 생긴다.
     */
...
private:
     ...
     *
     * @warning 주기는 설정값이 아니라 카메라가 통보한 세션 타임아웃에서 유도된다.
     */
    void keepAliveLoop();
     ...
     *
     * @details
     * RTSP Session 헤더는 Session: <id>[;timeout=<초>] 형식이고,
     * timeout은 이 시간 안에 요청이 없으면 세션을 끊겠다는 카메라의 통보다.
     *
     * 예전에는 이 값을 통째로 버리고 rtspKeepAliveIntervalSec(기본 30초) 고정 주기로만
     * GET_PARAMETER를 보냈다. 카메라가 timeout=10을 통보하면 30초짜리 keep-alive는
     * 영원히 늦어서, 스트림이 10초마다 끊기고 재연결되는 루프에 빠진다.
     * -- PLAY는 매번 성공하므로 인증/URI 문제처럼 보이지 않아 원인을 찾기 어렵다.
     *
     * 통보된 타임아웃의 절반을 주기로 삼는다. (최소 1초) 절반인 이유는 한 번 유실돼도
     * 다음 keep-alive가 만료 전에 도달하기 때문이다. 설정값이 더 짧으면 설정값을 쓴다.
     *
     ...
     * @return  헤더 길이 (RTP가 아니거나 길이가 모순이면 0)
     */
     ...
     * - connectTimeoutSec_   : SO_RCVTIMEO는 connect()에 적용되지 않음
     *                          논블로킹 connect + select()로 명시적 제한을 걸지 않으면
     *                          카메라 무응답/방화벽 SYN drop 시
    ...
    /// @brief 카메라가 Session 헤더로 통보한 세션 타임아웃 (0 = 통보 없음)
    ...
    /**
     * @brief   메타데이터 RTP가 실려오는 인터리브 채널 번호
     * @details SETUP에서 interleaved=0-1을 요청하지만, 서버가 다른 채널을 배정할 수 있다.
     *          응답의 Transport 헤더를 파싱해 채워지며, 없으면 요청값 0을 유지한다.
     */
    int rtpChannel_ = 0;
    ...
    std::uint32_t cseq_ = 4;
    std::uint32_t nonceCount_ = 1;
