 ...
 * 경계 처리는 여기서 하지 않는다.
 * -- 호출부가 곧바로 clampToOutput으로 [0,1] 에 가두므로,
 * 화면 끝에 걸린 박스는 안쪽으로만 자라고 중심은 그만큼 안으로 밀린다.
 * 분기/할당이 없어 프레임당 비용이 상수다.
 */
...
AffineImageCoordinateMapper::AffineImageCoordinateMapper(double scaleX, double scaleY, double offsetX, double offsetY,
    ...
    // AppConfig가 이미 [1.0, 2.0]으로 clamp하지만, DI로 직접 넣는 경로(테스트/다른 조립부)까지
    // 막으려면 여기서도 거부해야 한다.
...
void AffineImageCoordinateMapper::map(std::vector<domain::DetectedObject>& objects, veda::ChannelId channelId) const {
    ...
    for (std::size_t readIdx = 0; readIdx < objects.size(); ++readIdx) {
        ...
        // 그런 객체가 필터에서 탈락해 Blur 없이 노출된다.
        // -- 프라이버시 기능이므로 넓은 쪽이 안전하다.
        // NaN/Inf는 확대 연산을 통과해도 그대로 NaN/Inf이므로 아래 유한성 검사가 여전히 잡는다.
