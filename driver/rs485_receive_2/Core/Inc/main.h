 ...
 *   채널 B : PA1(A1) 경광등 릴레이 + PA8(D7) 부저 + PA7(D11) NeoPixel
 *
 * 줄 조명은 채널마다 한 줄씩이고, 각 줄은 자기 채널의 위험도를 색으로 표시한다
 * (NONE 초록 / WARNING 노랑 / DANGER 빨강). 옆 채널의 상태에는 반응하지 않는다.
 *
...
/**
 * 채널 B NeoPixel(WS2812) 줄 조명의 DIN. Arduino 헤더의 D11 = PA7 이다.
 *
 * A 채널(PA6)과 같은 TIM3 를 쓰되 타이머 채널이 다르다 -- TIM3_CH2(AF2) + DMA1 Stream5
 * Channel5. 핀만 바꿔 쓸 수 없는 이유는 각 타이머 채널의 DMA 요청이 정해진 스트림 한
 * 자리에만 걸리기 때문이다(RM0368 Table 27). 비트 타이밍은 A 채널과 같은 타이머이므로
 * 당연히 같다.
 *
 * !! 두 채널이 타이머 하나를 공유하므로 동시에 송신할 수 없다 -- 카운터가 하나뿐이다.
 *    neopixel_show_solid() 가 블로킹이고 태스크 문맥에서만 불려 전송이 절대 겹치지
 *    않는다는 전제 위에서만 성립한다. 쉬는 채널은 CCR 이 0 이라 그동안 핀이 Low 로
 *    남는다(PWM 모드 1은 CNT < CCR 일 때만 High 라 CCR = 0 이면 영원히 Low).
 *
 * 배선: PA7 -> 74AHCT125 -> 330R -> NeoPixel DIN (레벨 시프터 /OE 는 GND).
 */
#define NEOPIXEL_B_Pin GPIO_PIN_7
#define NEOPIXEL_B_GPIO_Port GPIOA
