...
| `C:\Users\3-10\STM32\Rs485_receive_demo` | **Slave #1** (이 폴더) — `MY_SLAVE_ID=1` → CH1, CH2 |
...
```sh
diff Rs485_receive_demo/Core/Src/main.c rs485_receive_2/Core/Src/main.c
# -> 54번 줄 MY_SLAVE_ID 한 줄만 나와야 정상
```
...
- `ack_pending` 은 **슬롯 하나뿐**이다. 앞 ACK 가 안 나갔으면 명령은 적용하되 ACK 는 버린다.
...
```
RX "S1:CH1:R2" -> APPLIED (relay+buzzer+strip)
RX "S2:CH3:R2" -> dropped: another slave
