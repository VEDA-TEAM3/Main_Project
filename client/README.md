...
<img width="1920" height="1080" alt="수정본v2" src="https://github.com/user-attachments/assets/f35e1313-fd56-4893-b1a2-63e0907d16bb" />
...
구역은 **0개에서 최대 6개**입니다. 지도 도면의 커버리지 격자가 3열 x 2행이라 그보다 많은 구역은 그릴
자리가 없습니다. 더 늘리려면 `DigitalTwinMapSceneBuilder`의 격자부터 다시 설계해야 합니다.

갓 배포한 프로그램에는 구역이 하나도 없습니다(`app_config.example.json`의 `video.areas`,
`video.streams`, `digitalTwin.world.zones`가 모두 비어 있습니다). 이때는 영상 자리에 안내 문구만 뜨고
도면 6칸이 모두 `확장 예정`으로 남습니다. 아래 **구역 관리** 탭에서 첫 구역을 추가한 뒤 다시 시작하면
됩니다.
...
- `initialAreaId`를 적었다면 `areas`에 등록된 구역 ID여야 합니다. 키를 빼면 첫 구역으로 시작합니다
  (구역 관리 탭으로 첫 구역을 추가했을 때가 이 경우입니다).
...
`app_config.example.json`은 구역이 비어 있는 배포 기본값이므로, 위 규칙에 맞춰 제 1구역에
`channelIndex` 0~3, 제 2구역에 4~7 식으로 직접 채웁니다. 테스트 환경에서는 두 구역이 같은 RTSP URL을
참조해도 되지만, 운영 환경에서는 구역별 실제 CCTV 주소를 지정합니다.
