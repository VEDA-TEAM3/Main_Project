...
```json
"zones": [
    { "minX": -59.0, "minY": -14.0, "maxX": -40.0, "maxY": 5.0 },
    { "minX": 41.0, "minY": -14.0, "maxX": 60.0, "maxY": 5.0 }
]
```

**월드 상자는 정사각형이어야 합니다.** 객체가 그려지는 영역이 구역 셀 안의 정사각형(scene 158 x 158)이므로,
상자의 가로세로 비율이 1이 아니면 가로·세로 배율이 달라져 실제로 정사각형인 구역이 화면에서 찌그러집니다.
현장 실측이 나오면 **한 변 길이만** 바꾸면 됩니다(중심은 그대로 두고 `±side/2`).

| 도면 기준값 | scene 단위 |
| --- | ---: |
| 구역 셀 | 194 x 202 |
| 객체 영역 (월드 상자와 1:1 대응) | **158 x 158** |
| 구역 1 객체 영역 | (162, 72) ~ (320, 230) |
| 구역 2 객체 영역 | (364, 72) ~ (522, 230) |

현재 한 변은 19 m이므로 축척은 19 / 158 = **0.1203 m/scene 단위**입니다. 두 구역의 **상대 위치는 도면과
무관합니다.** 도면은 두 구역을 나란히 그리지만 실제로는 떨어진 별개 구역이므로, 구역 사이 거리는 실측값을
`minX`/`maxX`에 직접 넣습니다.
