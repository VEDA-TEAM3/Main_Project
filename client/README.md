...
`video.initialStartDelayMs`(기본 300)는 창이 처음 표시된 뒤 첫 수신기를 켜기까지의 여유이고,
`video.receiverStartSpacingMs`(기본 250)는 채널과 채널 사이의 간격입니다. 4채널이면 마지막 채널이
`300 + 3 x 250 = 1050ms`에 시작합니다.

간격의 목적은 **순차 접속이 아니라 동시 접속 방지**입니다. 수신기는 채널마다 전용 `QThread`에서 돌고
`start()`는 queued로 던져지므로, 간격은 파이프라인 구성을 시작하는 시점만 어긋나게 할 뿐 RTSP
handshake와 디코더 초기화는 그 뒤로 서로 겹쳐서 진행됩니다. 그래서 간격을 handshake 시간만큼 길게
잡을 이유가 없습니다 — 늘려 봐야 전체 시작만 그만큼 늦어집니다.

NVR이 동시 세션 개시를 거부하거나 시작 직후 첫 프레임이 자주 깨지는 현장이라면 이 두 값을 올립니다.
**재빌드 없이 설정 파일만 고치면 됩니다.**

### 구역 및 채널 확장

구역은 **최대 8개**입니다. 지도 도면의 커버리지 격자가 4열 x 2행이라 그보다 많은 구역은 그릴 자리가
...
- 탑뷰 도면의 구역 활성화. 커버리지 격자 8칸 중 앞에서부터 구역 수만큼이 활성 구역(네온 테두리, 채널
...
상자로 정규화합니다. 구역 상자는 `digitalTwin.world.zones`에 `video.areas` 개수와 똑같이 적습니다(최대 8개).
...
탑뷰 도면은 주차장 한 층 전체를 그리고, 그 위에 물리 CCTV 커버리지를 **4열 x 2행 격자**로 얹습니다
...
- **비밀번호는 PBKDF2-HMAC-SHA256**(사용자별 salt, 반복 횟수는 레코드마다 저장)로 저장합니다.
  반복 횟수를 나중에 올려도 기존 계정이 그대로 열립니다.
...
- **`ts`는 로컬 UTC 기준 -60초 ~ +5초 창 안이어야 합니다.** 미래 `ts` 하나가 `BlurProcessor`의 최신
  timestamp를 끌어올리면 뒤이어 오는 정상 metadata가 전부 과거로 취급돼 버려지고, **블러가 조용히 꺼진 채
  얼굴과 번호판이 그대로 표시됩니다.** 미래 쪽 한계는 이 때문에 `blur.historyMs`보다 작아야 합니다.
  새 요구사항은 아닙니다 — 블러는 서버 `ts`를 영상 PTS에 UTC로 맞추므로 두 시계가 이미 초 단위로 맞아
  있어야 동작합니다. **관제 PC와 서버의 NTP 동기화가 안 되어 있으면 여기서 프로토콜 오류로 드러납니다.**
