...
Item {
    ...
    Item {
        ...
        // ── 도면 정적 레이어 ───────────────────────────────────────────
        // 바닥·주차구획·벽·기둥·차막이·설비실은 창 크기가 바뀔 때 말고는 변하지 않습니다.
        // 그런데 QQuickWidget에는 부분 갱신이 없어서, 객체가 20Hz로 움직이거나 아래 위험
        // 테두리가 vsync마다 맥동할 때마다 이 Shape 전부가 다시 래스터화됩니다. 한 번 그려
        // 텍스처로 캐시해 두면 그 프레임들은 사각형 하나 그리는 비용으로 끝납니다.
        //
        // textureSize를 화면에 실제로 나오는 픽셀 수로 잡는 것이 핵심입니다. 비워 두면 plan
        // 좌표(866x460)로 래스터화한 뒤 stage의 scale로 확대되어 선과 글자가 뭉개집니다.
        // devicePixelRatio까지 곱해야 합니다 — planScale은 논리 좌표 배율이라 125% 배율 화면에서
        // 이걸 빼면 실제 픽셀의 80%로만 그린 뒤 늘리게 됩니다(실측 평균 오차 2.77/255 -> 0.30).
        // 둘 다 창 크기가 바뀔 때만 변하므로 텍스처 재생성도 그때만 일어납니다.
        Item {
            id: planStatic

            readonly property real texturePixelScale: root.planScale * Screen.devicePixelRatio

            anchors.fill: parent
            layer.enabled: true
            layer.smooth: true
            layer.textureSize: Qt.size(Math.max(1, Math.ceil(root.plan.width * planStatic.texturePixelScale)),
                                       Math.max(1, Math.ceil(root.plan.height * planStatic.texturePixelScale)))
            ...
            PlanWalls {
                paths: planStatic.stallOutlines
                barWidth: 3.2
                relief: 0.0
            }
    ...
    Item {
        ...
        //
        // **min은 0으로 내리지 않습니다.** 맥동의 바닥이 0이면 위험이 유지되는 중에도 매 주기
        // 1초 가까이 테두리가 사실상 사라져서, 그때 화면을 본 사람은 위험 표시를 놓칩니다.
        // 바닥을 올려 두면 계속 붉게 남고 맥동은 밝기 차이로만 읽힙니다.
        Repeater {
            model: [
                {"w": 18, "min": 0.16, "max": 0.42, "c": "#ff0a1e"},
                {"w": 10, "min": 0.24, "max": 0.62, "c": "#f5081c"},
                {"w": 4.6, "min": 0.40, "max": 0.90, "c": "#ff1626"},
                {"w": 2.0, "min": 0.55, "max": 1.0, "c": "#ff2f3d"}
            ]
