...
# 2026-08-06 RTSP 안전성 변경의 성능 영향

- `sendAll()`은 SETUP, PLAY, keep-alive 제어 요청에서만 실행되며 부분 송신과 `EINTR`를 처리한다. 프레임 수신
  hot path에는 추가 반복이 없다.
- 소켓 fd mutex는 연결 공개, 취소, 종료에만 사용된다. 스트림 프레임 조립은 기존 사용자 공간 버퍼를 그대로
  사용한다.
- 재연결 backoff 포화 계산은 O(1)이며 실패 세션당 한 번 실행된다. 설정은 1~3600초로 제한된다.
- TLS가 기본값이 되면서 위의 2026-07-21 평문 RTSP 수치는 TLS 운용 성능을 대표하지 않는다. TLS handshake는
  연결당 한 번이고 스트림 수신은 `SSL_read`를 사용한다. 동일 카메라와 하드웨어에서 재측정하기 전에는 처리율
  변화량을 단정하지 않는다.

재측정 시 `metricsReportIntervalMs=5000`으로 Network/Source 로그의 프레임 수, 평균 조립·큐 지연, recv 호출 수,
FPS, 처리량을 각각 10개 이상 구간에서 수집한다. `rtspUseTls=true`와 `false`는 같은 입력 스트림에서 분리해 비교한다.